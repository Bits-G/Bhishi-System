...
import { Search, ShieldCheck, CheckCircle2, AlertTriangle, ArrowLeft, Briefcase } from "lucide-react";
...
export default function UpdateBusinessInfoPage() {
  ...
  async function selectMember(m: MemberHit) {
    setSelected(m);
    setResults([]);

    const { data: existing } = await supabase.from("business_directory").select("*").eq("member_id", m.id).maybeSingle();
    if (existing) {
      setInfoType((existing.info_type as any) ?? "");
      setWorkName(existing.work_name ?? "");
      setWorkRole(existing.work_role ?? "");
      setBusinessName(existing.business_name ?? "");
      setBusinessRole(existing.business_role ?? "");
      setBusinessMobile(existing.business_mobile_no ?? "");
      setPlace(existing.place ?? "");
      setBusinessArea(existing.business_area ?? "");
      setLandmark(existing.landmark ?? "");
      setCity(existing.city ?? "");
      setState(existing.state ?? "");
      setPincode(existing.pincode ?? "");
      const status = existing.work_status ?? "";
      ...
      setAdditionalInfo(existing.additional_info ?? "");
    }
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch("/api/business-directory/submit", {
      ...
      body: JSON.stringify({
        alot_number: selected.alot_number,
        mobile_no: mobile,
        info_type: infoType,
        work_name: workName,
        work_role: workRole,
  ...
  if (success) {
    return (
      <div className="max-w-md mx-auto text-center py-16">
        ...
        <h1 className="text-xl font-bold text-ink-900">Submitted!</h1>
  ...
  return (
    <div className="max-w-lg mx-auto">
      <a
        ...
        className="text-brand-700 text-sm font-medium flex items-center gap-1 mb-4 hover:underline w-fit"
      >
      ...
      <div className="text-center mb-6">
        ...
        <p className="text-ink-700/60 text-sm mt-1">Add or update your own work/business details for the public directory.</p>
      </div>
      ...
      {selected && !verified && (
        <div className="card space-y-3">
          <button
            type="button"
            onClick={() => { setSelected(null); setMobile(""); setVerifyError(""); }}
            className="text-brand-700 text-sm font-medium flex items-center gap-1 hover:underline"
          >
          ...
          <form onSubmit={handleVerify} className="space-y-3">
            <div>
              ...
              <input
                ...
                placeholder="The number on your member record"
              />
            </div>
      ...
      {selected && verified && (
        <form onSubmit={handleSubmit} className="card space-y-4">
          <p className="text-sm text-ink-800">
            Editing for Alot No <b>{selected.alot_number}</b> — <b>{selected.member_name}</b>
          </p>
