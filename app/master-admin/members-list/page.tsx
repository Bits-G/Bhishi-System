...
import { Pencil, X, Search, Trash2, AlertTriangle } from "lucide-react";
...
export default function MasterAdminMembersListPage() {
  ...
  function toggleAll() {
    setSelected((prev) => {
      if (allFilteredSelected) {
        // unselect only the currently filtered rows
        const next = new Set(prev);
  ...
  async function handleDeleteSelected() {
    ...
    const confirmed = confirm(
      `Permanently delete ${selected.size} member(s)? This also removes their attendance, payments, and winner records. This cannot be undone.`
    );
    if (!confirmed) return;
    setDeleting(true);
    const { error } = await supabase.from("members").delete().in("id", Array.from(selected));
    setDeleting(false);
    if (error) {
      alert(error.message);
      return;
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
        <div>
          ...
          <p className="text-ink-700/60 text-sm">
            {members.length} members registered. Master Admin has full edit &amp; delete control.
          </p>
        </div>
      ...
      {selected.size > 0 && (
        <div className="card border-rose-300 bg-rose-50 mb-4 flex items-center justify-between flex-wrap gap-3">
          <p className="text-sm text-rose-700 flex items-center gap-2 font-medium">
            <AlertTriangle size={16} /> {selected.size} member(s) selected
          </p>
          <button
            onClick={handleDeleteSelected}
            disabled={deleting}
            className="bg-rose-600 hover:bg-rose-700 text-white text-sm font-medium px-4 py-2 rounded-lg flex items-center gap-2"
          >
            <Trash2 size={16} /> {deleting ? "Deleting..." : `Delete Selected (${selected.size})`}
          </button>
        </div>
      )}
      ...
      <div className="card overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={7} className="py-6 text-center text-ink-700/50">No members found.</td>
              </tr>
            )}
          </tbody>
        </table>
